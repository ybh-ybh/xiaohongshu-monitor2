...
- ⚡ **商品级实时监控**: 列表中开启开关后，商品在每次查询完成后按 10 秒间隔继续查询；间隔可在设置中调整为 2-60 秒
...
3. 在监控系统中点击"添加商品"
4. 粘贴链接并读取商品信息
5. 在 SKU 弹窗中选择“监控整商品”，或取消该选项后勾选需要监控的 SKU 组合
6. 保存监控配置。商品列表中的“SKU”按钮可以随时调整已监控的组合
7. 新增商品默认开启“实时监控”；关闭后恢复按设置页中的全局分钟间隔刷新

指定 SKU 监控时，系统使用小红书返回的真实 SKU 编号和组合库存状态；只有选中的 SKU 有货时才会将商品标记为有货并触发补货通知。未指定 SKU 时保持原有的整商品监控行为。

商品列表的“实时监控”开关只影响当前商品。开启后不参与全局分钟任务，每次查询完成后才开始计算下一次间隔；设置页中的实时监控间隔支持 2 到 60 秒，默认 10 秒。
...
- `REAL_TIME_CHECK_INTERVAL_SECONDS`: 实时监控默认间隔（秒，默认 10；设置页可修改为 2-60）
